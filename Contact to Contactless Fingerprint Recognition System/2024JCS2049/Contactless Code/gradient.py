...
from part1.Contactless.thinning import skeletonize
...
def orientationField(input_img):
    block_size = 16

    (segmented_img, normim, mask) = create_segmented_and_variance_images(input_img, block_size, 0.2)

    mask = np.ones_like(input_img)

    angles = calculate_angles(input_img, W=block_size, smoth=False)
    orientation_img = visualize_angles(input_img, mask, angles, W=block_size)
...
if __name__ == '__main__':
    ...
    imgDir = './stn_output/*'
    outputDir = './orientationImages/'
    def open_images(directory):
        images_paths = glob(directory)
        return np.array([cv.imread(img_path,0) for img_path in images_paths])
    

    images = open_images(imgDir)

    os.makedirs(outputDir, exist_ok=True)
    for i, img in enumerate(tqdm(images)):
        results = orientationField(img)
        cv.imwrite(outputDir+str(i)+'.png', results)
